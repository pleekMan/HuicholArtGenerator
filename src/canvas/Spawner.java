...
public class Spawner {
	...
	int id;
	boolean[] directions = { true, false,false, false,false, false };
	...
	public Spawner(int _id) {
		...
		/*
		 * directions = new boolean[6]; for (int i = 0; i < directions.length;
		 * i++) { directions[i] = true; } //directions[0] = true;
		 */
	...
	public void spawn() {
		...
		for (int i = 0; i < 6; i++) { // FIX SO THAT IT ONLY CYCLES THROUGH
										// ACTIVE DIRECTIONS
	...
	public void render() {
		...
		for (int i = 0; i < points.size(); i++) {
			...
			//if(actualPoint.atStage() >= 2)
			/*for (int i = 0; i < 6; i++) { 
				
				
				int nextDir = i != 5 ? i + 1 : 0;
				if (directions[nextDir] == true) {
